...
4. 全部完成后核对产出：`weekly/<end_date>/AI-trends.md`、`AI-trends.html`、`weekly/<end_date>/ai-trends/clusters.json`、`weekly/index.html`。
...
§5 cluster_articles.py --end-date
    ...
    → clusters.json（聚类结果）
§6 render_ai_trends.py --start-date --end-date
...
# 确认已打印两段日期，且存在目录 weekly/$end_date/ai-trends/
```
...
扫描路径：`weekly/<end_date>/ai-trends/sites/*/articles/*.meta.json`。  
...
- [ ] `weekly/<end_date>/AI-trends.md` 与 `AI-trends.html` 已更新
- [ ] `weekly/<end_date>/ai-trends/clusters.json` 存在（本周跑过 §5）
