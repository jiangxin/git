...
test_expect_success "create tag with env, and gc works with lock" '
	(
		cd "$bare" &&
		GIT_REFS_TXN_NO_HOOK=1 git tag -m v1.0 v1.0 refs/heads/master &&
...
test_expect_success "clone and create local branches/tags (set env to bypass)" '
	GIT_REFS_TXN_NO_HOOK=1 git clone --no-local "$bare" work &&
	GIT_REFS_TXN_NO_HOOK=1 create_commits_in work H I J K &&
	(
		cd work &&
		GIT_REFS_TXN_NO_HOOK=1 git tag -m v1.1 v1.1 $H &&
		GIT_REFS_TXN_NO_HOOK=1 git tag -m v1.2 v1.2 $I &&
		GIT_REFS_TXN_NO_HOOK=1 git tag -m v1.3 v1.3 $J &&
		GIT_REFS_TXN_NO_HOOK=1 git branch dev $K
	)
'
